...
if __name__ == '__main__':
    # type [python pendulum_wCV_totalLogger.py] in your conda terminal to run this code
    ...
    model_RPPOwO = RecurrentPPO.load(f"./models/RPPOwO_checkpoint_500000.zip", env=env_si, device=device)

    # Load the mean reward logger's episode rewards
    with open(f"./logs/RPPOwO_total_rewards_500000.pkl", "rb") as f:
        total_reward_logger_RPPOwO.episode_rewards = pickle.load(f)
    # ===========================================================================================================

    # Train the model in 4 parts, saving the model and the logger at each milestone
    '''
    If I have none ->  range(5)
    If I have ~500000 -> range(1,5)
    If I have ~1000000 -> range(2,5)
    If I have ~1500000 -> range(3,5)
    If I have ~2000000 -> range(4,5)
    '''
    for i in range(1,5):
        ...
        model_RPPOwO.learn(total_timesteps=milestone, callback=total_reward_logger_RPPOwO, reset_num_timesteps=False)
        ...
        with open(f"./logs/RPPOwO_total_rewards_{current_timesteps}.pkl", "wb") as f:
            pickle.dump(total_reward_logger_RPPOwO.episode_rewards, f)

        print(
            f'RecurrentPPO/O Training Part {i + 1} End - Model and rewards saved at {current_timesteps} timesteps')
