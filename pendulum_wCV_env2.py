'''
This script trains and tests both the RecurrentPPO and PPO models for the Partially Observable Environment(no ang_vel).
'''
...
class RemoveVelocityWrapper(gym.ObservationWrapper):
    def __init__(self, env):
        super(RemoveVelocityWrapper, self).__init__(env)
        # 관찰 공간을 조정: angular velocity를 제거 (2개의 값만 남김)
        self.observation_space = gym.spaces.Box(low=-1.0, high=1.0, shape=(2,), dtype=np.float32)

    def observation(self, observation):
        # cos(θ)와 sin(θ)만 반환 (angular velocity 제거)
...
if __name__ == '__main__':
    # type [python pendulum_wCV_env2.py] in your conda terminal to run this code


    # define the environment

    env_so = gym.make('Pendulum-v1')
    env_so = RemoveVelocityWrapper(env_so)
    ...
    # Define the Model
    model_RPPOwO = RecurrentPPO('MlpLstmPolicy', env_so, verbose=1, gamma=0.9, learning_rate=0.001, gae_lambda=0.95)
